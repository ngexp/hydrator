...
## About
This library "hydrates" data, that means it adds data to an object from an incoming data source, like a json string or an array. The data source can be of almost any type, since the library provides and can extend various adapters. The data can also be constrained or modified by using attributes set on pre-defined class structures.

This library is for projects where strict type checking is wanted, if you want a quick automatic dump of any json into an object instance then there are plenty of other tools and libraries for that. This library does not do ANY type casting by default, but supplies attributes that can cast and modify values in different ways. 

The usage is simple, create a class structure with types and attributes, hydrate the data using one of the provided adapters or create a new one, and it'll create a new instance of the specified class with the required data.
...
Example:
```php
#[Size(min: 2, max: 30, message: "Size of name must be between {min} and {max}")]
private string $name;
```
